...
- Statements: `PRINT` (multiple items; trailing `;` or `,` suppresses newline), `INPUT` (multiple targets, including array elements), `LET` (or implicit assignment), `IF ... THEN <line>`, `GOTO`, `GOSUB` / `RETURN`, `FOR ... TO ... [STEP ...]` / `NEXT`, `END`, `REM`, `DIM`
